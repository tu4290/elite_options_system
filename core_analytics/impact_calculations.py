...
try:
    ...
    if hasattr(ids, 'COL_IMPACT_VEGA_RAW'):
        print(f"DEBUG_IMPACT_CALC: ids.COL_IMPACT_VEGA_RAW exists! Value: '{getattr(ids, 'COL_IMPACT_VEGA_RAW')}'")
    else:
        print("DEBUG_IMPACT_CALC: CRITICAL - ids.COL_IMPACT_VEGA_RAW DOES NOT EXIST in the imported 'ids' module.")
        # You could even force an error here to be absolutely sure this part is running before the AttributeError
        # raise AttributeError("DEBUG_IMPACT_CALC: Manually raising error because COL_IMPACT_VEGA_RAW is missing in ids")
    ...
    class ids: # type: ignore
        ...
        COL_IMPACT_SMI_RAW = 'smi'; COL_IMPACT_VPI_RAW = 'vpi'; COL_IMPACT_VPI_NORM = 'vpi_norm'; COL_IMPACT_PROXIMITY = 'proximity'
        COL_IMPACT_COMPOSITE_RAW = 'composite_impact'
    ...
    class system_utilities_fallback: # type: ignore
        ...
        # return df, False # In real code, you might return False
                return df, not missing # Corrected fallback
        @staticmethod
        ...
        def normalize_series(series: pd.Series, name: str, min_denominator: float) -> pd.Series:
            # Ensure log_instance is defined or handled if IMPORTS_SUCCESSFUL is False
            _log_instance = logger # Use module logger as a fallback
            if hasattr(system_utilities_fallback, 'log_instance_ref'): # Check if a logger was passed
                 _log_instance = system_utilities_fallback.log_instance_ref # type: ignore
            if _log_instance: _log_instance.debug(f"Dummy normalize_series called for {name}.")
            return series if isinstance(series, pd.Series) else pd.Series(dtype=float)

    # Store a reference to logger for fallback utilities if they are used
    system_utilities_fallback.log_instance_ref = logger # type: ignore
...
def calculate_delta_impact(
    ...
    Output column: ids.COL_IMPACT_DELTA_RAW
    """
    ...
    if not cols_ok:
        ...
        result_df[ids.COL_IMPACT_DELTA_RAW] = 0.0
        return result_df

    if ids.COL_IMPACT_PROXIMITY not in result_df.columns:
        result_df[ids.COL_IMPACT_PROXIMITY] = calculate_proximity_factor(
            result_df[strike_col], current_price, delta=result_df[option_delta_col], log_instance=impact_logger
        )
    result_df[ids.COL_IMPACT_PROXIMITY] = pd.to_numeric(result_df[ids.COL_IMPACT_PROXIMITY], errors='coerce').fillna(0.0)
    ...
    proximity_num = result_df[ids.COL_IMPACT_PROXIMITY]

    result_df[ids.COL_IMPACT_DELTA_RAW] = 0.0
    call_mask = result_df[opt_kind_col].astype(str).str.lower() == 'call'
    put_mask = result_df[opt_kind_col].astype(str).str.lower() == 'put'

    result_df.loc[call_mask, ids.COL_IMPACT_DELTA_RAW] += (deltas_buy_num[call_mask] * buy_w * proximity_num[call_mask])
    result_df.loc[put_mask, ids.COL_IMPACT_DELTA_RAW] += (deltas_sell_num[put_mask] * sell_w * proximity_num[put_mask])
    result_df.loc[call_mask, ids.COL_IMPACT_DELTA_RAW] -= (deltas_sell_num[call_mask] * sell_w * proximity_num[call_mask])
    result_df.loc[put_mask, ids.COL_IMPACT_DELTA_RAW] -= (deltas_buy_num[put_mask] * buy_w * proximity_num[put_mask])

    result_df[ids.COL_IMPACT_DELTA_RAW] = result_df[ids.COL_IMPACT_DELTA_RAW].fillna(0.0)
    impact_logger.debug(f"Delta Impact calculated. Example: {result_df[ids.COL_IMPACT_DELTA_RAW].head(1).to_string(index=False) if not result_df.empty else 'N/A'}")
    return result_df
...
def calculate_gamma_impact(
    ...
    if not cols_ok: result_df[ids.COL_IMPACT_GAMMA_RAW] = 0.0; return result_df
    if ids.COL_IMPACT_PROXIMITY not in result_df.columns:
        result_df[ids.COL_IMPACT_PROXIMITY] = calculate_proximity_factor(result_df[strike_col], current_price, delta=result_df[option_delta_col], log_instance=impact_logger)
    result_df[ids.COL_IMPACT_PROXIMITY] = pd.to_numeric(result_df[ids.COL_IMPACT_PROXIMITY], errors='coerce').fillna(0.0)
    ...
    prox_num = result_df[ids.COL_IMPACT_PROXIMITY]
    result_df[ids.COL_IMPACT_GAMMA_RAW] = ((gs_num * sell_w) - (gb_num * buy_w)) * prox_num # Dealer perspective: selling gamma is positive for their book
    result_df[ids.COL_IMPACT_GAMMA_RAW] = result_df[ids.COL_IMPACT_GAMMA_RAW].fillna(0.0)
    impact_logger.debug(f"Gamma Impact calculated. Example: {result_df[ids.COL_IMPACT_GAMMA_RAW].head(1).to_string(index=False) if not result_df.empty else 'N/A'}")
    return result_df
...
def calculate_vega_impact(
    ...
    if not cols_ok: result_df[ids.COL_IMPACT_VEGA_RAW] = 0.0; return result_df
    if ids.COL_IMPACT_PROXIMITY not in result_df.columns:
        result_df[ids.COL_IMPACT_PROXIMITY] = calculate_proximity_factor(result_df[strike_col], current_price, delta=result_df[option_delta_col], log_instance=impact_logger)
    result_df[ids.COL_IMPACT_PROXIMITY] = pd.to_numeric(result_df[ids.COL_IMPACT_PROXIMITY], errors='coerce').fillna(0.0)
    ...
    prox_num = result_df[ids.COL_IMPACT_PROXIMITY]
    result_df[ids.COL_IMPACT_VEGA_RAW] = ((vb_num * buy_w) - (vs_num * sell_w)) * prox_num
    result_df[ids.COL_IMPACT_VEGA_RAW] = result_df[ids.COL_IMPACT_VEGA_RAW].fillna(0.0)
    impact_logger.debug(f"Vega Impact calculated. Example: {result_df[ids.COL_IMPACT_VEGA_RAW].head(1).to_string(index=False) if not result_df.empty else 'N/A'}")
    return result_df
...
def calculate_theta_impact(
    ...
    if not cols_ok: result_df[ids.COL_IMPACT_THETA_RAW] = 0.0; return result_df
    if ids.COL_IMPACT_PROXIMITY not in result_df.columns:
        result_df[ids.COL_IMPACT_PROXIMITY] = calculate_proximity_factor(result_df[strike_col], current_price, delta=result_df[option_delta_col], log_instance=impact_logger)
    result_df[ids.COL_IMPACT_PROXIMITY] = pd.to_numeric(result_df[ids.COL_IMPACT_PROXIMITY], errors='coerce').fillna(0.0)
    ...
    prox_num = result_df[ids.COL_IMPACT_PROXIMITY]
    result_df[ids.COL_IMPACT_THETA_RAW] = ((tb_num * buy_w) - (ts_num * sell_w)) * prox_num
    result_df[ids.COL_IMPACT_THETA_RAW] = result_df[ids.COL_IMPACT_THETA_RAW].fillna(0.0)
    impact_logger.debug(f"Theta Impact calculated. Example: {result_df[ids.COL_IMPACT_THETA_RAW].head(1).to_string(index=False) if not result_df.empty else 'N/A'}")
    return result_df
...
def calculate_volume_impact(
    ...
    if not cols_ok: result_df[ids.COL_IMPACT_VOLUME_RAW] = 0.0; return result_df
    if ids.COL_IMPACT_PROXIMITY not in result_df.columns:
        result_df[ids.COL_IMPACT_PROXIMITY] = calculate_proximity_factor(result_df[strike_col], current_price, delta=result_df[option_delta_col], log_instance=impact_logger)
    result_df[ids.COL_IMPACT_PROXIMITY] = pd.to_numeric(result_df[ids.COL_IMPACT_PROXIMITY], errors='coerce').fillna(0.0)
    ...
    call_mask = result_df[opt_kind_col].astype(str).str.lower() == 'call'
    put_mask = result_df[opt_kind_col].astype(str).str.lower() == 'put'
    result_df[ids.COL_IMPACT_VOLUME_RAW] = 0.0
    result_df.loc[call_mask, ids.COL_IMPACT_VOLUME_RAW] += (vb_num[call_mask] * buy_w * prox_num[call_mask])
    result_df.loc[put_mask, ids.COL_IMPACT_VOLUME_RAW] += (vs_num[put_mask] * sell_w * prox_num[put_mask])
    result_df.loc[call_mask, ids.COL_IMPACT_VOLUME_RAW] -= (vs_num[call_mask] * sell_w * prox_num[call_mask])
    result_df.loc[put_mask, ids.COL_IMPACT_VOLUME_RAW] -= (vb_num[put_mask] * buy_w * prox_num[put_mask])
    result_df[ids.COL_IMPACT_VOLUME_RAW] = result_df[ids.COL_IMPACT_VOLUME_RAW].fillna(0.0)
    impact_logger.debug(f"Volume Impact calculated. Example: {result_df[ids.COL_IMPACT_VOLUME_RAW].head(1).to_string(index=False) if not result_df.empty else 'N/A'}")
    return result_df
...
def calculate_value_impact(
    ...
    if not cols_ok: result_df[ids.COL_IMPACT_VALUE_RAW] = 0.0; return result_df
    if ids.COL_IMPACT_PROXIMITY not in result_df.columns:
        result_df[ids.COL_IMPACT_PROXIMITY] = calculate_proximity_factor(result_df[strike_col], current_price, delta=result_df[option_delta_col], log_instance=impact_logger)
    result_df[ids.COL_IMPACT_PROXIMITY] = pd.to_numeric(result_df[ids.COL_IMPACT_PROXIMITY], errors='coerce').fillna(0.0)
    ...
    call_mask = result_df[opt_kind_col].astype(str).str.lower() == 'call'
    put_mask = result_df[opt_kind_col].astype(str).str.lower() == 'put'
    result_df[ids.COL_IMPACT_VALUE_RAW] = 0.0
    result_df.loc[call_mask, ids.COL_IMPACT_VALUE_RAW] += (vb_num[call_mask] * buy_w * prox_num[call_mask])
    result_df.loc[put_mask, ids.COL_IMPACT_VALUE_RAW] += (vs_num[put_mask] * sell_w * prox_num[put_mask])
    result_df.loc[call_mask, ids.COL_IMPACT_VALUE_RAW] -= (vs_num[call_mask] * sell_w * prox_num[call_mask])
    result_df.loc[put_mask, ids.COL_IMPACT_VALUE_RAW] -= (vb_num[put_mask] * buy_w * prox_num[put_mask])
    result_df[ids.COL_IMPACT_VALUE_RAW] = result_df[ids.COL_IMPACT_VALUE_RAW].fillna(0.0)
    impact_logger.debug(f"Value Impact calculated. Example: {result_df[ids.COL_IMPACT_VALUE_RAW].head(1).to_string(index=False) if not result_df.empty else 'N/A'}")
    return result_df
...
def calculate_composite_impact(
    ...
    config_key_to_raw_col_map = {
        "delta_impact_norm": ids.COL_IMPACT_DELTA_RAW, # Config key for weight vs. raw column name
        "gamma_impact_norm": ids.COL_IMPACT_GAMMA_RAW,
        "vega_impact_norm": ids.COL_IMPACT_VEGA_RAW,
        "theta_impact_norm": ids.COL_IMPACT_THETA_RAW,
        "volume_impact_norm": ids.COL_IMPACT_VOLUME_RAW,
        "value_impact_norm": ids.COL_IMPACT_VALUE_RAW
    }
    ...
    if not cols_ok:
        ...
        result_df[ids.COL_IMPACT_COMPOSITE_RAW] = 0.0
        return result_df
    ...
    for norm_key_in_config, raw_col_name_in_df in config_key_to_raw_col_map.items():
        ...
        if normalize_components:
            ...
            result_df[ids.COL_IMPACT_COMPOSITE_RAW] += weight * component_series_normalized
        else:
            impact_logger.debug(f"Composite Impact: Adding raw component '{raw_col_name_in_df}' without normalization. Weight: {weight:.2f}")
            result_df[ids.COL_IMPACT_COMPOSITE_RAW] += weight * component_series
    ...
    if normalize_components:
        if total_weight_applied > MIN_NORMALIZATION_DENOMINATOR_IMPACT and abs(total_weight_applied - 1.0) > 0.01:
            ...
            result_df[ids.COL_IMPACT_COMPOSITE_RAW] /= total_weight_applied
        ...
        result_df[ids.COL_IMPACT_COMPOSITE_RAW] = normalize_series(result_df[ids.COL_IMPACT_COMPOSITE_RAW], "FinalCompositeImpactRawSource", min_denominator=MIN_NORMALIZATION_DENOMINATOR_IMPACT)

    result_df[ids.COL_IMPACT_COMPOSITE_RAW] = result_df[ids.COL_IMPACT_COMPOSITE_RAW].fillna(0.0)
    impact_logger.debug(f"Composite Impact (Normalized Components: {normalize_components}) calculated. Example: {result_df[ids.COL_IMPACT_COMPOSITE_RAW].head(1).to_string(index=False) if not result_df.empty else 'N/A'}")
    return result_df
...
def calculate_strike_magnetism(
    ...
    if not cols_ok:
        ...
        result_df[ids.COL_IMPACT_SMI_RAW] = 0.0
        return result_df
    ...
    result_df[ids.COL_IMPACT_SMI_RAW] = (
        gxoi_numeric * np.sign(dxoi_numeric.replace(0,1e-9)) * oi_change_factor * result_df[ids.COL_IMPACT_PROXIMITY]
    ).fillna(0.0)
    impact_logger.debug(f"SMI calculated. Example: {result_df[ids.COL_IMPACT_SMI_RAW].head(1).to_string(index=False) if not result_df.empty else 'N/A'}")
    return result_df
...
def calculate_volatility_pressure(
    ...
    if not cols_ok:
        ...
        result_df[ids.COL_IMPACT_VPI_RAW] = 0.0
        result_df[ids.COL_IMPACT_VPI_NORM] = 0.0
        return result_df
    ...
    proximity_numeric = result_df[ids.COL_IMPACT_PROXIMITY]

    result_df[ids.COL_IMPACT_VPI_RAW] = (vega_impact_numeric * np.sign(gamma_impact_numeric.replace(0,1e-9)) * proximity_numeric).fillna(0.0)
    result_df[ids.COL_IMPACT_VPI_NORM] = normalize_series(result_df[ids.COL_IMPACT_VPI_RAW], "VPINorm", min_denominator=MIN_NORMALIZATION_DENOMINATOR_IMPACT)
    impact_logger.debug(f"VPI calculated. Example Raw: {result_df[ids.COL_IMPACT_VPI_RAW].head(1).to_string(index=False) if not result_df.empty else 'N/A'}, Norm: {result_df[ids.COL_IMPACT_VPI_NORM].head(1).to_string(index=False) if not result_df.empty else 'N/A'}")
    return result_df
...
def detect_volatility_regime(
    ...
    if ids.COL_IMPACT_PROXIMITY not in data.columns:
        regime_logger.debug(f"Proximity column '{ids.COL_IMPACT_PROXIMITY}' not found. Calculating it now.")
        data[ids.COL_IMPACT_PROXIMITY] = calculate_proximity_factor(
            data[strike_col], current_price, delta=data[option_delta_col], log_instance=regime_logger
        )
    data[ids.COL_IMPACT_PROXIMITY] = pd.to_numeric(data[ids.COL_IMPACT_PROXIMITY], errors='coerce').fillna(0.0)
